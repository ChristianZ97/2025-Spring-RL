...
class Policy(nn.Module):
    ...
    def forward(self, state):
        ...
        obs1 = F.tanh(self.obs_bn1(self.obs_layer1(state)))
        obs2 = F.tanh(self.obs_bn2(self.obs_layer2(obs1)))

        act1 = F.tanh(self.act_bn1(self.act_layer1(obs2)))
        action_prob = self.act_layer2(act1)

        val1 = F.tanh(self.val_bn1(self.val_layer1(obs2)))
    ...
    def select_action(self, state):
        ...
        self.saved_actions.append(SavedAction(m.log_prob(action), state_value, m.entropy()))
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        for (log_prob_action, state_value, entropy), R, advantage in zip(saved_actions, returns, advantages):
        ...
        loss = torch.stack(policy_losses).sum() + torch.stack(value_losses).sum() - entropy_coef * entropy_term
...
def train(lr=0.01, lambda_=0.95, entropy_coef=0.01):
    """
    ...
    model.entropy_coef = entropy_coef
    ...
    for i_episode in count(1):
        ...
        writer.add_scalar('Train/Entropy', entropy_term.item(), i_episode)
